...
const RestaurantList = ({route}:any) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            while(!!nextPageToken) {
                let url = "https://maps.googleapis.com/maps/api/place/textsearch/json?key=AIzaSyBCZVSjuVPDGigu-jFRtmV6p8fVXtlOxyc&pagetoken="+nextPageToken
            ...
            <View style={styles.viewStyle} key={idx} onTouchEnd={() => navigation.navigate("MainPage", {restaurantName: item.name})}>
    ...
    return (
        <View style={{paddingHorizontal: 15, paddingVertical: 20, backgroundColor: 'white', width: '100%', flex: 1}}>
            <MapView style={{height: 160, marginBottom: 10}} region={{ latitude: citylat, longitude: citylng, latitudeDelta: 0.01, longitudeDelta: 0.01 }} >
